...
/**
 * Kafka Configuration for bhpubwrt (Producer/Aggregator) module.
 * Configures:
 * - Producer for publishing payloads (TSValues[])
 * - Producer for publishing status
 * - Consumer for consuming status replies from 3 consumer groups
 */
@EnableKafka
@Configuration
public class BhpubwrtKafkaConfig {
    ...
    // Producer for TSValues[] payloads
    @Bean
    ...
    // Producer for PayloadStatus (used by bhpubwrt to send status)
    @Bean
    ...
    // Multi-cluster producer factories (optional - for additional clusters)
    @Bean
    ...
    // Consumer for PayloadStatus replies (3 consumer groups)
    @Bean
